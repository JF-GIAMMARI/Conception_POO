package tp2_collections.exercice_5;
...
public class Personne implements Comparable<Personne> {
    ...
    /**
     * Redéfnition de CompateTo avec prise en compte d'abords de la date de naissance puis de l'ordre alphabétique du nom/prenom
     **/
    @Override
    public int compareTo(Personne obj) {
        int result = 0; // Default value
        if (obj.dateNaissance.compareTo(this.dateNaissance) < 0)
            result = -1;
        else if (obj.dateNaissance.compareTo(this.dateNaissance) > 0)
            result = 1;
        else {
            result = this.nom.compareTo(obj.nom);
            if (result == 0){
                result = this.prenom.compareTo(obj.prenom);
            }
